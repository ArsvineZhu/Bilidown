...
FFmpeg 8.1.2 与 LAME 3.100 从固定源码和校验和构建，目标最低 macOS 13。产物为 `Bilidown-<版本>-macos-arm64.app.zip` 或 `...-x86_64.app.zip`。默认使用 ad-hoc 签名。

## GitHub Actions

`CI` 在 PR 和 main/master 推送上运行 pytest、TypeScript、Vitest、Vite 与 Playwright。`Portable builds` 可手动触发；未填写版本时使用 `dev-<短 SHA>`，三个包和第三方源码作为 artifacts 保留 14 天。

推送与项目版本完全一致的 `vX.Y.Z` 标签创建正式 Release；`vX.Y.Z-rc.N` 仅在项目版本本身也是该预发布版本时创建 prerelease。Release 包含三个便携 ZIP、第三方源码包和 `SHA256SUMS.txt`。
...
确认 CI 全绿、三个包均通过 `packaging/smoke-portable.py`、macOS 架构和最低版本正确、签名状态符合预期、许可证与源码归档齐全。真实 Bilibili/4K 测试在本机执行，不作为 Release 阻塞型 CI。
